...
@section('konten')
    ...
    <form action="{{ route('kehadiran.saveKehadiran') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="datetime-local" name="jam_masuk" class="form-control" id="exampleInputEmail1"
                aria-describedby="emailHelp" value="{{ now()->setTimezone('T')->format('Y-m-dTh:m') }}">
            <small id="emailHelp" class="form-text text-muted">example : Kaprodi</small>
        </div>
        <div class="form-group">
            <label for="exampleInputPassword1">Jam Keluar</label>
            <input type="datetime-local" name="jam_keluar" class="form-control" id="exampleInputPassword1"
